...
# Verifique se há conexão com a internet antes de tentar atualizar
if (Test-InternetConnection) {
    Write-Host "Conexão com a internet detectada. Iniciando os updates..."

    # Garantir que o Chocolatey esteja atualizado
    if (Get-Command choco -ErrorAction SilentlyContinue) {
        Write-Host "Atualizando o Chocolatey..."
        choco upgrade chocolatey -y
    }

    # Atualize o PowerShell para a versão mais recente, se necessário
    if (Get-Command choco -ErrorAction SilentlyContinue) {
        Write-Host "Atualizando o PowerShell Core..."
        choco upgrade powershell-core -y
    }

    Write-Host "Atualizações concluídas!"
} else {
    Write-Host "Nenhuma conexão com a internet detectada. Atualizações não realizadas."
}
